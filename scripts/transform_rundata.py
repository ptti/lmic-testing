...
for line in vals:
    ...
    Casenames[str(int(line[0])+1)] += (' {} Self-isolation'.format(line[3]))
    Casenames[str(int(line[0])+1)] += ((' Unlimited' if line[4] != '500' else ' Limited') + ' Availability.') # m
    Casenames[str(int(line[0])+1)] += ((' No' if line[5] == '0.0' else '') + ' Clinical Testing, ')
    Casenames[str(int(line[0])+1)] += ((' No' if line[6] == '0.0' else '') + ' Release Testing')
    Casenames[str(int(line[0])+1)] += ((' No' if line[7] == '0.0' else '') + ' Wild Testing')
